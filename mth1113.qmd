...
- well-defined
$\begin{aligned} \sum_{k=1}^{\infty} p_X(k) & =\sum_{k=1}^{\infty}(1-p)^{k-1} p \\ & =p \sum_{k=0}^{\infty}(1-p)^k \\ & =p \cdot \frac{1}{1-(1-p)} \\ & =1 .\end{aligned}$
...
the memoryless property is saying that given the
first n trials are unsuccessful, the number of trials until success after
the first n trials has the same distribution as the unconditional number of trials until success (independent)
